...
(function (udf){
  ...
  function len(a){
    if (arrp(a) || strp(a) || fnp(a))return len_(a);
    if (objp(a)){
      var n = 0;
      for (var k in a){
  ...
  function begStr(a, x){
    ...
    var l = ag.length;
    for (var i = 1; i < l; i++){
      if (beg1Str(a, ag[i]))return true;
    }
    return false;
  }
  
  function beg1(a, x){
    if (strp(a))return beg1Str(a, x);
    if (arrp(a) || htmp(a))return fst(a) === x;
    err(beg1, "Can't find if a = $1 begs with x = $2", a, x);
  }
  
  function beg1Str(a, x){
    if (strp(x))return beg1StrStr(a, x);
    if (rgxp(x))return beg1StrRgx(a, x);
    err(beg1Str, "Can't find if str a = $1 begs with x = $2", a, x);
  }
  
  function beg1StrStr(a, x){
    var lx = x.length;
    var la = a.length;
  ...
  function beg1StrRgx(a, x){
    return posStrRgx(x, a) === 0;
  }
  ...
  function endStr(a){
    ...
    var l = ag.length;
    for (var i = 1; i < l; i++){
      if (end1Str(a, ag[i]))return true;
    }
    return false;
  }
  
  function end1(a, x){
    if (strp(a))return end1Str(a, x);
    if (arrp(a) || htmp(a))return las(a) === x;
    err(end1, "Can't find if a = $1 ends with x = $2", a, x);
  }
  
  function end1Str(a, x){
    if (strp(x))return end1StrStr(a, x);
    //if (rgxp(x))return end1sx(a, x);
    err(end1Str, "Can't find if str a = $1 ends with x = $2", a, x);
  }
  
  function end1StrStr(a, x){
    var lx = x.length;
    var la = a.length;
    if (lx > la)return false;
  ...
  */
  ...
  att({
    ...
    beg1: beg1,
    beg1Str: beg1Str,
    beg1StrStr: beg1StrStr,
    beg1StrRgx: beg1StrRgx,
    end: end,
    endStr: endStr,
    end1: end1,
    end1Str: end1Str,
    end1StrStr: end1StrStr,
    bnd: bnd,
